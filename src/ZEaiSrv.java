...
//******************************************************************************
public class ZEaiSrv implements JCO.ServerExceptionListener, JCO.ServerStateChangedListener {
//******************************************************************************
	...
	static {
		...
		//------------------------------------------------------------------------------
		//  Add function 'ZEAI_ECHOCLI'
		//------------------------------------------------------------------------------
		fmeta = new JCO.MetaData("ZEAI_ECHOCLI");
		fmeta.addInfo("RESNO" , JCO.TYPE_CHAR, 20,   0,     0,       JCO.IMPORT_PARAMETER, null);
	...
	static public class Server extends JCO.Server {
		...
		protected void handleRequest(JCO.Function function)
		{
			...
			if (function.getName().equals("STFC_CONNECTION")) {
				...
				try	{
					System.out.println("From SAP : " + input.getString("REQUTEXT"));
				}
				catch (Exception e)	{	}
				output.setValue("This is a response from ZEaiSrv.java","RESPTEXT");
			}
			else if (function.getName().equals("ZEAI_ECHOCLI")) {
				String strRegNo = "";
				String strReturn = "";
				...
				try	{
					strRegNo = input.getString("RESNO");
					System.out.println("From SAP : " + strRegNo);
					clientSocket = new Socket("203.254.225.39", 9004);
					outToServer = new DataOutputStream(clientSocket.getOutputStream());
					inFromServer = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
					outToServer.writeBytes(strRegNo + '\n');
					strReturn = inFromServer.readLine();
					clientSocket.close();
					output.setValue(strReturn,"RETURN");
					//output.setValue("OK","RETURN");
					System.out.println("To SAP : " + strReturn);
				}
				catch (Exception e)	{
					output.setValue(e.toString(),"RESPONSE");
					output.setValue("ER","RETURN");
				}
				finally {
					try	{ 
	...
	public void startServers()
	{
		...
		srv[0] = new Server("42.1.55.49","sapgw00","YEAISRV",repository);
		// Server 2 listens for incoming requests from system 2
		// (Change gateway host, service, and program ID according to your needs)
		srv[1] = new Server("42.1.55.49","sapgw00","ZEAISRV",repository);
